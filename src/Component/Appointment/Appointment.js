...
function Appointment() {
    ...
    const [prevData, setPrevData] = useState({ contact: "", email: "" });
    ...
    const fetchUser = async (e) => {
        ...
        try {
            const response = await fetch("/findPrevUser", {
                method: "POST",
                body: requestJson,
            });
            const responseText = await response.text();
            setUser(JSON.parse(responseText))
            setFormData(JSON.parse(responseText))
        ...
        try {
            if (formData.patient_type === 'new') {
                ...
                setFormData({
                 ...
                 response = await fetch("/updateAppointment", {
                    method: "POST",
                    body: requestJson,
                });
            }
        ...
        <section>
                                ...
                                <div className='row'>
                                    <div className='col  m-3'>
                                        ...
                                        <input type="date" class="form-control" id="admitdate"
                                            name='date'
                                            value={formData.date}
                                            onChange={(e) => setFormData({ ...formData, last_visite_date: formData.date, date: e.target.value })}
                                        />
            ...
            <Modal show={show} onHide={handleClose}>
                ...
                <Modal.Body>
                    ...
                    <label className='m-3'> Contact </label>
                    <input type="String" placeholder="your contact number  " className='m-3' min="1"
                        name='contact'
                        value={prevData.contact}
                        onChange={(e) => setPrevData({ ...prevData, contact: e.target.value })}
                    />
                </Modal.Body>
                <Modal.Footer>
